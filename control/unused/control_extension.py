...
class MoveItIkDemo:
	...
	def draw_circles(self, xy_center_pos, radius = 0.10):
		...
		for t in range(self.cont):
			self.target_pose.header.frame_id = self.reference_frame
			self.target_pose.header.stamp = rospy.Time.now()
			self.target_pose.pose.position.x = xy_center_pos[0] + radius * np.cos( 2 * np.pi * t / self.cont)
			self.target_pose.pose.position.y = xy_center_pos[1] + radius * np.sin( 2 * np.pi * t / self.cont)
...
if __name__ == "__main__":
	demo = MoveItIkDemo()
	point = [0, -0.3]
	demo.draw_circle_v3(point, 0.03)
